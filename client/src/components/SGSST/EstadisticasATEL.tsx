    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-wrap items-center gap-2 p-2 rounded-xl bg-surface-secondary border border-border-medium shadow-sm">
                ...
                {/* Generar IA */}
                <button onClick={() => handleGenerate('ANNUAL')} disabled={isGenerating} className="group flex items-center px-4 py-2 bg-teal-600 hover:bg-teal-700 border border-teal-600 hover:border-teal-700 text-white rounded-full transition-all duration-300 shadow-sm shrink-0 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                ...
                {/* Guardar Informe */}
                <button onClick={() => handleSaveReport()} disabled={!editorContent && !generatedReport} className="group flex items-center px-4 py-2 bg-violet-600 hover:bg-violet-700 border border-violet-600 text-white rounded-full transition-all duration-300 shadow-sm shrink-0 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                ...
                {(editorContent || generatedReport) ? (
                    <ExportDropdown content={editorContent || generatedReport || ''} fileName={`Estadisticas_ATEL_${MONTHS[currentMonthIndex]}`} />
                ) : (
