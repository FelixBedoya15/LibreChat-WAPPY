  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 p-2 rounded-xl bg-surface-secondary border border-border-medium shadow-sm">
                {/* Historial */}
                <button onClick={() => setIsHistoryOpen(!isHistoryOpen)} className={`flex items-center justify-center w-10 h-10 border border-border-medium rounded-full transition-all duration-300 shadow-sm shrink-0 ${isHistoryOpen ? 'bg-teal-100 text-teal-700' : 'bg-surface-primary text-text-primary hover:bg-surface-hover'}`}>
                ...
                {/* Guardar Informe */}
                <button onClick={() => handleSave()} disabled={!editorContent && !generatedReport} className="group flex items-center px-4 py-2 bg-violet-600 hover:bg-violet-700 border border-violet-600 text-white rounded-full transition-all duration-300 shadow-sm shrink-0 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                ...
                {(editorContent || generatedReport) ? (
                    <ExportDropdown content={editorContent || generatedReport || ''} fileName={"Evaluacion_Ergonomica_OWAS"} />
                ) : (
