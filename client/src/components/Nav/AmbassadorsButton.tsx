...
const AmbassadorsButton = ({
  ...
  const isActive = location.pathname.startsWith('/embajadores');

  const handleClick = () => {
    navigate('/embajadores/dashboard');
  ...
  if (isCollapsed) {
    return (
      <TooltipAnchor
        description="Métricas & Embajadores"
        side="right"
        render={
          <motion.button
            whileTap={{ scale: 0.95 }}
  ...
  return (
    ...
    >
      <Award className="h-4 w-4 shrink-0 text-teal-600 dark:text-teal-400" />
      <span className="font-semibold text-text-primary text-[13px] flex-1 text-left">Métricas & Embajadores</span>
    </motion.button>
  );
};

export default AmbassadorsButton;
