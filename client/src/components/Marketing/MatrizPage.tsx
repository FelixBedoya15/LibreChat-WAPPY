...
export default function MatrizPage() {
  ...
  const funnelKey = 'matriz';

  // State variables for Matriz dates and WhatsApp redirects
  ...
  // Temporary states for Matriz Date campaign
  ...
  const fetchConfig = async () => {
    try {
      ...
      if (response.data) {
        ...
        // Fetch Matriz Date campaign options
        if (data.matrizDate1 !== undefined) { setMatrizDate1(data.matrizDate1); setTempDate1(data.matrizDate1); }
        if (data.matrizWhatsappUrl1 !== undefined) { setMatrizWhatsappUrl1(data.matrizWhatsappUrl1); setTempWhatsappUrl1(data.matrizWhatsappUrl1); }
        if (data.matrizDate2 !== undefined) { setMatrizDate2(data.matrizDate2); setTempDate2(data.matrizDate2); }
        if (data.matrizWhatsappUrl2 !== undefined) { setMatrizWhatsappUrl2(data.matrizWhatsappUrl2); setTempWhatsappUrl2(data.matrizWhatsappUrl2); }
        if (data.matrizDate3 !== undefined) { setMatrizDate3(data.matrizDate3); setTempDate3(data.matrizDate3); }
        if (data.matrizWhatsappUrl3 !== undefined) { setMatrizWhatsappUrl3(data.matrizWhatsappUrl3); setTempWhatsappUrl3(data.matrizWhatsappUrl3); }
      ...
      <div>
            ...
            <div className="bg-surface-primary/95 border border-emerald-500/40 rounded-2xl p-6 backdrop-blur-md shadow-xl text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="border-t border-border-medium/60 my-4 pt-4">
                    <h4 className="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-3">Campaña Matriz: Fechas y Grupos de WhatsApp</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            placeholder="Ej: Martes 17 de Junio - 7:00 PM"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-semibold text-text-secondary mb-1">Enlace WhatsApp Grupo 1</label>
                          <input
                            type="text"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-xl bg-surface-secondary border border-border-medium text-text-primary text-xs focus:outline-none focus:border-emerald-500 transition-all"
                            placeholder="Ej: Jueves 19 de Junio - 7:00 PM"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-semibold text-text-secondary mb-1">Enlace WhatsApp Grupo 2</label>
                          <input
                            type="text"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-xl bg-surface-secondary border border-border-medium text-text-primary text-xs focus:outline-none focus:border-emerald-500 transition-all"
                            placeholder="Ej: Sábado 21 de Junio - 4:00 PM"
                          />
                        </div>
                        <div>
                          <label className="block text-[10px] font-semibold text-text-secondary mb-1">Enlace WhatsApp Grupo 3</label>
                          <input
                            type="text"
        ...
        {configLoading ? (
          ...
          <main className="w-full max-w-4xl mx-auto px-6 py-4 flex flex-col items-center text-center relative z-10">
            ...
            {funnelKey === 'matriz' && isVideoFinished && (
              <div className="w-full max-w-5xl mx-auto mt-12 px-4 text-center">
                {/* Embedded style block for keyframes and floating animations */}
                <style dangerouslySetInnerHTML={{ __html: `
                  ...
                  @keyframes pulse-glow-emerald {
                    0%, 100% { filter: drop-shadow(0 0 15px rgba(16,185,129,0.25)); }
                    50% { filter: drop-shadow(0 0 25px rgba(16,185,129,0.55)); }
                  }
                  @keyframes pulse-glow-teal {
                    0%, 100% { filter: drop-shadow(0 0 15px rgba(20,184,166,0.25)); }
                    50% { filter: drop-shadow(0 0 25px rgba(20,184,166,0.55)); }
                  }
                  @keyframes pulse-glow-cyan {
                    0%, 100% { filter: drop-shadow(0 0 15px rgba(6,182,212,0.25)); }
                    50% { filter: drop-shadow(0 0 25px rgba(6,182,212,0.55)); }
                  }
                  .animate-float-avatar-1 {
                    animation: float-gentle-1 6s ease-in-out infinite;
                  }
                  .animate-float-avatar-2 {
                    animation: float-gentle-2 7s ease-in-out infinite;
                  }
                  .animate-float-avatar-3 {
                    animation: float-gentle-3 5.5s ease-in-out infinite;
                  }
                  .avatar-glow-emerald {
                    animation: pulse-glow-emerald 3.5s ease-in-out infinite;
                  }
                  .avatar-glow-teal {
                    animation: pulse-glow-teal 4s ease-in-out infinite;
                  }
                  .avatar-glow-cyan {
                ...
                {/* Floating Avatar Decorations on the background sides (Desktop only) */}
                <div className="hidden lg:block absolute left-4 xl:left-12 top-[60%] w-24 h-24 pointer-events-none z-10 animate-float-avatar-1 opacity-70">
                ...
                <div className="hidden lg:block absolute right-4 xl:right-12 top-[65%] w-24 h-24 pointer-events-none z-10 animate-float-avatar-2 opacity-70">
                  <div className="relative group">
                ...
                {/* Notice / Invitation Banner */}
                <div className="mx-auto my-6 p-5 bg-gradient-to-r from-emerald-500/10 via-teal-500/15 to-cyan-500/10 border border-emerald-500/30 rounded-3xl max-w-4xl flex flex-col md:flex-row items-center justify-between gap-4 text-left shadow-lg backdrop-blur-md">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm sm:text-base font-extrabold text-text-primary tracking-tight">
                        🔴 ¡OFERTA DE LANZAMIENTO EXCLUSIVA DISPONIBLE!
                      </h3>
                      <p className="text-xs text-text-secondary mt-0.5">
                        Separa tu cupo seleccionando una de las 3 opciones de fecha. Serás redirigido al grupo privado de WhatsApp de esa sesión.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 animate-bounce bg-emerald-500/10 text-emerald-500 text-xs font-bold px-3 py-1 rounded-full border border-emerald-500/20">
                ...
                <h2 className="text-3xl font-black text-text-primary mb-2 tracking-tight">
                  SEPARAR MI PLAZA AL CURSO
                </h2>
                <p className="text-sm text-text-secondary max-w-2xl mx-auto mb-10">
                  Selecciona la fecha y hora que mejor se adapte a tu agenda para unirte al grupo exclusivo de WhatsApp del curso, donde compartiremos el enlace de acceso directo y los materiales de trabajo.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto text-left relative z-20">
                  ...
                  <div className="group relative flex flex-col rounded-3xl border bg-gradient-to-b p-6 sm:p-8 transition-all duration-500 hover:-translate-y-2 from-emerald-500/5 to-emerald-500/10 border-emerald-500/20 hover:border-emerald-500/40 hover:shadow-[0_0_50px_rgba(16,185,129,0.15)] bg-surface-primary/60 backdrop-blur-md">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div className="text-center mb-6">
                        <p className="text-xs text-text-secondary uppercase font-semibold tracking-wider">Fecha Seleccionada</p>
                        <h4 className="text-xl font-black text-text-primary mt-1 min-h-[56px] flex items-center justify-center px-2">
                      ...
                      <button
                        onClick={() => {
                          const url = matrizWhatsappUrl1 || whatsappUrl;
                          window.open(url, '_blank', 'noopener,noreferrer');
                          axios.post('/api/comunidad/metrics/session', { sessionId: localStorage.getItem('wappy_comunidad_session') || '', clickType: 'whatsapp_group_1', funnelKey }).catch(() => {});
                        }}
                        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-400 hover:to-emerald-500 py-3.5 text-xs font-black text-slate-950 shadow-lg shadow-emerald-500/10 transition-all hover:scale-[1.03] active:scale-[0.98] duration-300 uppercase tracking-wider"
                      >
                        <svg className="w-4 h-4 fill-current mr-2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.262 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.625 1.451 5.403.002 9.803-4.394 9.806-9.799.002-2.593-1.002-5.031-2.83-6.861-1.827-1.83-4.269-2.829-6.865-2.83-5.408 0-9.807 4.394-9.81 9.8.002 1.547.459 3.054 1.32 4.394l-.99 3.616 3.744-.972zm8.482-6.249c-.183-.092-1.09-.539-1.26-.6-.17-.061-.29-.092-.41.092-.12.184-.47.6-.57.72-.1.12-.2.13-.38.04-.18-.09-.77-.284-1.47-.908-.54-.485-.905-1.085-1.01-1.27-.1-.18-.01-.277.08-.368.08-.08.18-.21.27-.31.09-.1.12-.17.18-.3.06-.13.03-.24-.015-.33-.045-.09-.41-1-.56-1.37-.15-.356-.3-.308-.41-.313-.105-.005-.23-.005-.35-.005-.12 0-.32.045-.49.23-.17.18-.65.64-.65 1.56 0 .92.67 1.81.76 1.94.1.13 1.31 2.01 3.19 2.82.44.19.8.31 1.07.39.45.14.86.12 1.18.07.36-.05 1.09-.45 1.24-.88.15-.43.15-.8 0-.88-.04-.08-.16-.12-.34-.21z"/>
                        </svg>
                        Separar Plaza en WhatsApp
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="group relative flex flex-col rounded-3xl border bg-gradient-to-b p-6 sm:p-8 transition-all duration-500 hover:-translate-y-2 from-teal-500/5 to-teal-500/10 border-teal-500/20 hover:border-teal-500/40 hover:shadow-[0_0_50px_rgba(20,184,166,0.15)] bg-surface-primary/60 backdrop-blur-md">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <button
                        onClick={() => {
                          const url = matrizWhatsappUrl2 || whatsappUrl;
                          window.open(url, '_blank', 'noopener,noreferrer');
                          axios.post('/api/comunidad/metrics/session', { sessionId: localStorage.getItem('wappy_comunidad_session') || '', clickType: 'whatsapp_group_2', funnelKey }).catch(() => {});
                        }}
                        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-400 hover:to-teal-500 py-3.5 text-xs font-black text-slate-950 shadow-lg shadow-teal-500/10 transition-all hover:scale-[1.03] active:scale-[0.98] duration-300 uppercase tracking-wider"
                      >
                        <svg className="w-4 h-4 fill-current mr-2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.262 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.625 1.451 5.403.002 9.803-4.394 9.806-9.799.002-2.593-1.002-5.031-2.83-6.861-1.827-1.83-4.269-2.829-6.865-2.83-5.408 0-9.807 4.394-9.81 9.8.002 1.547.459 3.054 1.32 4.394l-.99 3.616 3.744-.972zm8.482-6.249c-.183-.092-1.09-.539-1.26-.6-.17-.061-.29-.092-.41.092-.12.184-.47.6-.57.72-.1.12-.2.13-.38.04-.18-.09-.77-.284-1.47-.908-.54-.485-.905-1.085-1.01-1.27-.1-.18-.01-.277.08-.368.08-.08.18-.21.27-.31.09-.1.12-.17.18-.3.06-.13.03-.24-.015-.33-.045-.09-.41-1-.56-1.37-.15-.356-.3-.308-.41-.313-.105-.005-.23-.005-.35-.005-.12 0-.32.045-.49.23-.17.18-.65.64-.65 1.56 0 .92.67 1.81.76 1.94.1.13 1.31 2.01 3.19 2.82.44.19.8.31 1.07.39.45.14.86.12 1.18.07.36-.05 1.09-.45 1.24-.88.15-.43.15-.8 0-.88-.04-.08-.16-.12-.34-.21z"/>
                        </svg>
                  ...
                  <div className="group relative flex flex-col rounded-3xl border bg-gradient-to-b p-6 sm:p-8 transition-all duration-500 hover:-translate-y-2 from-cyan-500/5 to-cyan-500/10 border-cyan-500/20 hover:border-cyan-500/40 hover:shadow-[0_0_50px_rgba(6,182,212,0.15)] bg-surface-primary/60 backdrop-blur-md">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      ...
                      <button
                        onClick={() => {
                          const url = matrizWhatsappUrl3 || whatsappUrl;
                          window.open(url, '_blank', 'noopener,noreferrer');
                          axios.post('/api/comunidad/metrics/session', { sessionId: localStorage.getItem('wappy_comunidad_session') || '', clickType: 'whatsapp_group_3', funnelKey }).catch(() => {});
                        }}
                        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-500 to-cyan-600 hover:from-cyan-400 hover:to-cyan-500 py-3.5 text-xs font-black text-slate-950 shadow-lg shadow-cyan-500/10 transition-all hover:scale-[1.03] active:scale-[0.98] duration-300 uppercase tracking-wider"
                      >
                        <svg className="w-4 h-4 fill-current mr-2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.262 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.625 1.451 5.403.002 9.803-4.394 9.806-9.799.002-2.593-1.002-5.031-2.83-6.861-1.827-1.83-4.269-2.829-6.865-2.83-5.408 0-9.807 4.394-9.81 9.8.002 1.547.459 3.054 1.32 4.394l-.99 3.616 3.744-.972zm8.482-6.249c-.183-.092-1.09-.539-1.26-.6-.17-.061-.29-.092-.41.092-.12.184-.47.6-.57.72-.1.12-.2.13-.38.04-.18-.09-.77-.284-1.47-.908-.54-.485-.905-1.085-1.01-1.27-.1-.18-.01-.277.08-.368.08-.08.18-.21.27-.31.09-.1.12-.17.18-.3.06-.13.03-.24-.015-.33-.045-.09-.41-1-.56-1.37-.15-.356-.3-.308-.41-.313-.105-.005-.23-.005-.35-.005-.12 0-.32.045-.49.23-.17.18-.65.64-.65 1.56 0 .92.67 1.81.76 1.94.1.13 1.31 2.01 3.19 2.82.44.19.8.31 1.07.39.45.14.86.12 1.18.07.36-.05 1.09-.45 1.24-.88.15-.43.15-.8 0-.88-.04-.08-.16-.12-.34-.21z"/>
                        </svg>
